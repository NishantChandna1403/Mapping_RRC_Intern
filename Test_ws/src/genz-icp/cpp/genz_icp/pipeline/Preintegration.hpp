// genz_icp/cpp/genz_icp/pipeline/Preintegration.hpp
#ifndef GENZ_ICP_PREINTEGRATION_HPP_
#define GENZ_ICP_PREINTEGRATION_HPP_
...
public:
    ...
    struct PreintegratedMeasurement {
        ...
        Vector3d delta_velocity = Vector3d::Zero();
        double delta_time = 0.0;
    };

    Preintegration(const Vector3d &acc0, const Vector3d &gyr0,
                   const Vector3d &linearized_ba, const Vector3d &linearized_bg,
                   double acc_n = 0.00059, double gyr_n = 0.000061,
                   double acc_w = 0.000011, double gyr_w = 0.000001,
        ...
        : dt_(0.0),
          ...
          acc_buf_(),
          gyr_buf_(),
          g_vec_(-Eigen::Vector3d(0, 0, gravity)),
          nf(0.0),
          cf(0.0),
          acc_n_(acc_n),
          gyr_n_(gyr_n),
    ...
    // Move constructor
    Preintegration(Preintegration&& other) noexcept
        : dt_(other.dt_),
          acc0_(std::move(other.acc0_)),
          gyr0_(std::move(other.gyr0_)),
          acc1_(std::move(other.acc1_)),
          gyr1_(std::move(other.gyr1_)),
          linearized_acc_(std::move(other.linearized_acc_)),
          linearized_gyr_(std::move(other.linearized_gyr_)),
          linearized_ba_(std::move(other.linearized_ba_)),
          linearized_bg_(std::move(other.linearized_bg_)),
          jacobian_(std::move(other.jacobian_)),
          covariance_(std::move(other.covariance_)),
          noise_(std::move(other.noise_)),
          sum_dt_(other.sum_dt_),
          delta_p_(std::move(other.delta_p_)),
          delta_q_(std::move(other.delta_q_)),
          delta_v_(std::move(other.delta_v_)),
          dt_buf_(std::move(other.dt_buf_)),
          acc_buf_(std::move(other.acc_buf_)),
          gyr_buf_(std::move(other.gyr_buf_)),
          g_vec_(std::move(other.g_vec_)),
          nf(other.nf),
          cf(other.cf),
          acc_n_(other.acc_n_),
          gyr_n_(other.gyr_n_),
    ...
    Preintegration& operator=(Preintegration&& other) noexcept {
        if (this != &other) {
            dt_ = other.dt_;
            acc0_ = std::move(other.acc0_);
            gyr0_ = std::move(other.gyr0_);
    ...
    PreintegratedMeasurement GetMeasurement() const {
        PreintegratedMeasurement meas;
        meas.delta_pose = Sophus::SE3d(delta_q_, delta_p_);
        meas.delta_velocity = delta_v_;
        meas.delta_time = sum_dt_;
        return meas;
    }
    ...
    Matrix<double, 15, 1> evaluate(const Vector3d &Pi,
        ...
        Vector3d dbg = Bgi - linearized_bg_;
...
private:
    Quaterniond deltaQ(const Vector3d &theta) {
        Vector3d half_theta = theta * 0.5;
        double norm = half_theta.norm();
        if (norm < 1e-6) {
            return Quaterniond(1.0, 0.0, 0.0, 0.0);
        }
        double sin_norm = std::sin(norm) / norm;
        return Quaterniond(std::cos(norm), half_theta.x() * sin_norm,
                          half_theta.y() * sin_norm, half_theta.z() * sin_norm);
    }
...
public:
    ...
    Vector3d linearized_acc_; // Removed const to enable move semantics
    Vector3d linearized_gyr_; // Removed const to enable move semantics
...
#endif // GENZ_ICP_PREINTEGRATION_HPP_