...
struct IMUPreintegrationResidual {
    IMUPreintegrationResidual(const Eigen::Vector3d& delta_p,
                             const Eigen::Quaterniond& delta_q,
                             const Eigen::Vector3d& delta_v,
                             const Sophus::SE3d& prev_pose)
        : delta_p_(delta_p), delta_q_(delta_q), delta_v_(delta_v), prev_pose_(prev_pose) {}
    ...
    bool operator()(const T* const trans, const T* const quat, const T* const vel, T* residual) const {
        ...
        // Position residual
        Eigen::Matrix<T, 3, 1> p_res = prev_q.cast<T>() * (t - prev_t.cast<T>()) - delta_p_.cast<T>();
        residual[0] = p_res[0];
        residual[1] = p_res[1];
        residual[2] = p_res[2];

        // Rotation residual
        Eigen::Quaternion<T> q_diff = (prev_q.cast<T>() * delta_q_.cast<T>()).conjugate() * q;
        residual[3] = T(2.0) * q_diff.x();
        residual[4] = T(2.0) * q_diff.y();
        residual[5] = T(2.0) * q_diff.z();

        // Velocity residual
        Eigen::Matrix<T, 3, 1> v_res = prev_q.cast<T>() * (v - Eigen::Matrix<T, 3, 1>::Zero()) - delta_v_.cast<T>();
        residual[6] = v_res[0];
        residual[7] = v_res[1];
        residual[8] = v_res[2];
...
Sophus::SE3d GenZICP::fusePosesWithCeres(const Sophus::SE3d& icp_pose,
    ...
    std::cout << "[DEBUG] IMU Preintegration - Sum Dt: " << sum_dt
              << ", Delta P: " << delta_p.transpose()
              << ", Delta V: " << delta_v.transpose()
              << ", Delta Q: [" << delta_q.w() << ", " << delta_q.x() << ", " << delta_q.y() << ", " << delta_q.z() << "]" << std::endl;

    Eigen::Vector3d gps_trans = local_position_pose.translation();
    if (sum_dt > 0.0 && velocity.norm() > 1e-6) {
        gps_trans += velocity * sum_dt;
        std::cout << "[DEBUG] Applied velocity correction: " << (velocity * sum_dt).transpose() << std::endl;
    }
    ...
    if (sum_dt > 0.0 && !poses_.empty()) {
        problem.AddResidualBlock(
            new ceres::AutoDiffCostFunction<IMUPreintegrationResidual, 9, 3, 4, 3>(
                new IMUPreintegrationResidual(delta_p, delta_q, delta_v, poses_.back())),
            nullptr, trans, quat, vel);
        std::cout << "[DEBUG] Added IMU preintegration residual block" << std::endl;
    } else {
        std::cout << "[DEBUG] Skipped IMU residual block - Sum Dt: " << sum_dt
...
GenZICP::Vector3dVectorTuple GenZICP::RegisterFrame(const std::vector<Eigen::Vector3d> &frame,
                                                   ...
                                                   const Eigen::Matrix<double, 15, 15> &covariance,
                                                   double sum_dt) {
    std::cout << "[DEBUG] Entering RegisterFrame (preintegrated)" << std::endl;
    std::cout << "[DEBUG] Raw input frame size: " << frame.size() << std::endl;
    std::cout << "[DEBUG] Preintegrated input - Sum Dt: " << sum_dt
              << ", Delta P: " << delta_p.transpose()
              << ", Delta V: " << delta_v.transpose() << std::endl;
    ...
    try {
        ...
        if (first_frame_) {
            ...
            // Use preintegrated delta_q and delta_p for initial guess
            Sophus::SE3d preintegrated_guess(delta_q, delta_p);
            ...
            initial_guess = is_attitude_identity ? last_pose * preintegrated_guess : last_pose * attitude_delta;
            last_attitude_orientation_ = attitude_orientation;
        }
        ...
        new_pose = fusePosesWithCeres(icp_pose, attitude_orientation, local_position_pose, planar_points, non_planar_points, eph, epv, velocity, delta_p, delta_q, delta_v, jacobian, covariance, sum_dt);
...
GenZICP::Vector3dVectorTuple GenZICP::RegisterFrame(const std::vector<Eigen::Vector3d> &frame,
    ...
    return RegisterFrame(frame, timestamps, attitude_orientation, local_position_pose,
                         ...
                         Eigen::Vector3d::Zero(), Eigen::Quaterniond(1.0, 0.0, 0.0, 0.0),
                         Eigen::Vector3d::Zero(), Eigen::Matrix<double, 15, 15>::Identity(),
                         Eigen::Matrix<double, 15, 15>::Identity(), 0.0);
}
